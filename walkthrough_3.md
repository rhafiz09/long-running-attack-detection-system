...
### 🛡️ Phase 5: 100% PRD Perfection (Security & Deployment)
- [app/core/security.py](file:///d:/Projects/BilCode/ML%20-%20Pendeteksi%20Serangan%20Long%20Running%20Attack/codebase_new/app/core/security.py): API Key authentication dependency (`verify_api_key`) enforcing `X-API-Key` headers on sensitive endpoints.
- [app/core/limiter.py](file:///d:/Projects/BilCode/ML%20-%20Pendeteksi%20Serangan%20Long%20Running%20Attack/codebase_new/app/core/limiter.py): Shared SlowAPI rate limiter instance keyed by client IP.
- [scripts/backup_db.py](file:///d:/Projects/BilCode/ML%20-%20Pendeteksi%20Serangan%20Long%20Running%20Attack/codebase_new/scripts/backup_db.py): Automated database backup utility utilizing `pg_dump` and `gzip` to archive `.sql.gz` snapshots to `backups/`.
- [Dockerfile.fastapi](file:///d:/Projects/BilCode/ML%20-%20Pendeteksi%20Serangan%20Long%20Running%20Attack/codebase_new/Dockerfile.fastapi): Multi-stage slim Docker build containerizing the ML Engine & FastAPI service on port 8000.
- [Dockerfile.django](file:///d:/Projects/BilCode/ML%20-%20Pendeteksi%20Serangan%20Long%20Running%20Attack/codebase_new/Dockerfile.django): Slim Docker build containerizing the Django Dashboard on port 8001.
- [docker-compose.yml](file:///d:/Projects/BilCode/ML%20-%20Pendeteksi%20Serangan%20Long%20Running%20Attack/codebase_new/docker-compose.yml): Root orchestration file running `postgres_db`, `fastapi_engine`, and `django_dashboard` with automated healthchecks and persistent volumes.
...
## 🛡️ Phase 5: How to Run Security Tools & Docker Deployment

We have achieved **100% PRD Perfection** by completing all Application Security and Deployment features!

### 1. API Authentication & Rate Limiting
Sensitive endpoints (like `POST /api/v1/detect`) now require an `X-API-Key` header and enforce rate limits (`100/minute`).
To test via curl or python:
```powershell
curl -X POST "http://127.0.0.1:8000/api/v1/detect" -H "X-API-Key: soc-secret-api-key-2026" -H "Content-Type: application/json" -d "{\"logs\":[]}"
```

### 2. Automated Database Backups
Run the automated backup script at any time or via cron to generate compressed `.sql.gz` database snapshots inside `backups/`:
```powershell
python scripts/backup_db.py
```

### 3. Turnkey Docker Containerization
To launch the entire enterprise stack (PostgreSQL, FastAPI ML Engine, and Django Dashboard) in isolated Docker containers:
```powershell
